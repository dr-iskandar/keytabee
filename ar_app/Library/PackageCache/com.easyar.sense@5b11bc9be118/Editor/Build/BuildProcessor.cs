...
#error seperate binding cannot be used with this package
#else
                    PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/Binding/Apple/iOS/ios-arm64/easyar.framework", videoRecording || !hasNR);
                    if (hasNR)
                    {
                        PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/BindingNR/Apple/iOS/ios-arm64/easyar.framework", !videoRecording);
                    }
#endif
                    ...
                    if (permissions.Mega && string.IsNullOrEmpty(PlayerSettings.iOS.locationUsageDescription))
                    {
                ...
                {
                    PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/Android/permission.CAMERA.aar", permissions.CameraDevice);
                    PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/Android/permission.RECORD_AUDIO.aar", permissions.VideoRecording);
#if EASYAR_ENABLE_MEGA
                    PrepareOptionalNativePlugin($"Packages/com.easyar.mega/Runtime/Android/permission.ACCESS_FINE_LOCATION.aar", permissions.Mega);
#endif
                    ...
                    {
                        if (!settings.DisableARCoreAREngine)
                        {
                            EnsureGradleVersionIsSupported(settings);
                        }
                        if (settings.DisableARCoreAREngine)
                        {
                            Debug.LogWarning("ARCore and AREngine are manually disabled. See EasyAR > Sense > Configuration > Disable ARCore AREngine.");
                        }
                        else if (arcore != EasyARSettings.ARCoreType.External)
                        {
                    ...
                    if (arcoreLoader || (arcore != EasyARSettings.ARCoreType.External && !settings.DisableARCoreAREngine))
                    {
                    ...
                    PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/Android/com.google.ar.core-1.23.0.aar", !settings.DisableARCoreAREngine && arcore != EasyARSettings.ARCoreType.External);
                    PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/Android/com.google.ar.core-required.aar", !settings.DisableARCoreAREngine && arcore == EasyARSettings.ARCoreType.Required);
                    PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/Android/com.google.ar.core-optional.aar", !settings.DisableARCoreAREngine && arcore == EasyARSettings.ARCoreType.Optional);
                    PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/BindingAREngine/Android/AREngineInterop.aar", !settings.DisableARCoreAREngine && arengine != EasyARSettings.AREngineType.Disabled);
                    PrepareOptionalNativePlugin($"Packages/{UnityPackage.Name}/Runtime/BindingAREngine/Android/arenginesdk-3.7.0.3.aar", !settings.DisableARCoreAREngine && arengine == EasyARSettings.AREngineType.AREngineInterop);
                }
                AddShader();
        ...
        private static void EnsureGradleVersionIsSupported(EasyARSettings settings)
        {
            ...
            if (Gradle.TryGetVersion(out var gradleVersion, out var diagnosticMessage))
            {
                ...
                {
                    var errorMessage = $"ARCore and AREngine requires at least Gradle version {minimumGradleVersion} ({gradleVersion} detected). Visit https://developers.google.com/ar/develop/unity/android-11-build for further details.";
                    var selection = EditorUtility.DisplayDialogComplex(
                        "Gradle update required",
                        errorMessage,
                        "Cancel build", "Continue anyway", "Continue and disable both ARCore and AREngine");
                    ...
                    switch (selection)
                    {
                        ...
                        case 1:
                            break;
                        case 2:
                            settings.DisableARCoreAREngine = true;
                            EditorUtility.DisplayDialog(
                                "ARCore and AREngine Disabled",
                                "ARCore and AREngine are manually disabled. See EasyAR > Sense > Configuration > Disable ARCore AREngine.",
                                "OK"
                                );
                            break;
                    }
